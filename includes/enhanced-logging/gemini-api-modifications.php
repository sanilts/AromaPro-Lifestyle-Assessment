<?php
/**
 * Modified Gemini API Class with Enhanced Logging
 * 
 * These modifications should be added to the class-gemini-api.php file
 */
...
/**
 * Process a form submission with a prompt - Enhanced with detailed logging for Gemini
 *
 ...
 */
function cgptfc_enhanced_gemini_process_form_with_prompt($prompt_id, $form_data, $entry_id = null) {
    ...
    $timer_data = apply_filters('cgptfc_before_process_form_with_prompt_gemini', array(), $prompt_id, $form_data, $entry_id);
    ...
    if ($prompt_type === 'all_form_data') {
        // Use all form data
        $user_prompt = gemini_format_all_form_data($form_data, $prompt_id);
    } else {
        // Use custom template
        if (empty($user_prompt_template)) {
            ...
            do_action('cgptfc_after_process_form_with_prompt_gemini', $error, $prompt_id, $form_data, $entry_id, $timer_data);
    ...
    if (is_wp_error($response)) {
        ...
        // Log error if logging is enabled
        do_action('cgptfc_after_process_form_with_prompt_gemini', $response, $prompt_id, $form_data, $entry_id, $timer_data);
        
        return $response;
    }

    $content = gemini_get_response_content($response);
    ...
    do_action('cgptfc_after_process_form_with_prompt_gemini', $content, $prompt_id, $form_data, $entry_id, $timer_data);

    return $content;
}

/**
 * Format all form data for enhanced tracking for Gemini
 * 
 * @param array $form_data The form data
 * @param int $prompt_id The prompt ID
 * @return string Formatted form data text
 */
function gemini_format_all_form_data($form_data, $prompt_id) {
    $output = __('Here is the submitted form data:', 'chatgpt-fluent-connector') . "\n\n";

    // Get field labels if possible
    $field_labels = gemini_get_form_field_labels($prompt_id);
...
/**
 * Get form field labels for Gemini API
 * 
 * @param int $prompt_id The prompt ID
 * @return array Field labels
 */
function gemini_get_form_field_labels($prompt_id) {
    $field_labels = array();
